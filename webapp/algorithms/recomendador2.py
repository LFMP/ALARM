...
def adapter2(request,data): #Most accessed links
    recomendacoes = []
    links = Post.objects.exclude(href = data.href).values_list("href", flat = True).distinct() #todos os links presentes no banco de dados
    users = Post.objects.exclude(idClick = data.idClick).values_list("idUser", flat = True) #id de todos os usuários exceto ao que clicou o link
    ...
    for each_link in links:
        for each_user in users:
            ...
            if each_link in users_links: #verifica se o usuario acessou o link href e outro link qualquer
